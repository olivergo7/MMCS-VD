...
type_map = {
    ...
    'ElseStatement': 69 ,'ThrowStatement':70,'CFGExceptionNode':71
}
...
edgeType_full = {
    ...
    'DOM': 7,
    'POST_DOM': 8,
    'IS_FUNCTION_OF_AST': 9,
    'IS_FUNCTION_OF_CFG': 10
}
...
def inputGeneration(nodeCSV, edgeCSV, target, model, edge_type_map, cfg_only=False):
    gInput = dict()
    gInput["targets"] = list()
    gInput["graph"] = list()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--csv', help='normalized csv files to process', default='')
    parser.add_argument('--src', help='source c files to process', default='')
    # parser.add_argument('--wv', default='./model/wv_models/datasets')
    parser.add_argument('--output', default='../data1/ggnn_data/')
    parser.add_argument('--model_type', default=None, type=str, help="The model architecture to be fine-tuned.")
    parser.add_argument('--tokenizer_name', help='output path for sliced full data json', default='../processed_data/')
    args = parser.parse_args()

    json_file_path = '../processed_data/' + args.project + '_full_data_with_slices.json'
    data = json.load(open(json_file_path))
    config_class, model_class, tokenizer_class = MODEL_CLASSES[args.model_type]
    model = tokenizer_class.from_pretrained(args.tokenizer_name)
