...
# set the config
try:
    clang.cindex.Config.set_library_path("C:/Users/r1583/anaconda3/envs/pytorch/Lib/site-packages/LLVM/lib")
    clang.cindex.Config.set_library_file('C:/Users/r1583/anaconda3/envs/pytorch/Lib/site-packages/LLVM/bin/libclang.dll')
except:
    pass
...
def extract_nodes_with_location_info(nodes):
    ...
    for node_index, node in enumerate(nodes):
        ...
        if 'location' in node.keys():
            ...
            # key represent which number
            node_id = node['key'].strip()
            # node_indices represent physical index
...
def create_adjacency_list(line_numbers, node_id_to_line_numbers, edges, data_dependency_only=False):
    # return an array, array[0] represent control flow edges, array[1] represent data flow edges
    adjacency_list = {}
    ...
    for edge in edges:
        ...
        # if edge_type in ['IS_AST_PARENT', 'FLOWS_TO']:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--project', help='name of project for differentiating files', default='Fan')
    parser.add_argument('--input', help='directory where raw code and parsed are stored', default='../raw_data/Fan_c/')
    parser.add_argument('--text_in', help='path to cfg_full_text_files', default='../processed_data/ggnn_input/')
    parser.add_argument('--output', help='output path for sliced full data json', default='../processed_data/')
    args = parser.parse_args()

    split_dir = args.input
    print(split_dir)
    parsed = '../code-slicer/'+args.project+'_csv/'

    ggnn_json_data = json.load(open(args.text_in + args.project + '_cpg_full_text_files.json'))
    ...
    for i, file_name  in enumerate(files):
        ...
        nodes = []
        with open(nodes_file_path, mode='r', encoding='utf-8', errors='ignore') as nc:
            nodes_file = csv.DictReader(nc, delimiter='\t')
            try:
            # nodes：Store all parsed node types and values
        ...
        if len(nodes) == 0:
            continue
        # 当前获取函数调用，数组索引访问，指针访问 和 操作运算符的四种方法的行号.
        for node_idx, node in enumerate(nodes):
            ...
            # PtrMemberAccess: Pointer member access
