...
parser.add_argument('--base', help='paths to loaction of json shards to be ccombined', default='../data1/ggnn_data/')
parser.add_argument('--model', help='paths to word2vector model', default='./model/wv_models/datasets')
parser.add_argument('--output', help='output directory for resulting json file', default='../processed_data/')
...
def extract_graph_data(
    ...
    for sc in range(1, shard_count + 1):
        ...
        shard_file = open(os.path.join(base_dir, '.shard' + str(sc)))
        # print("path:", os.path.join(base_dir, project,project + '.json.shard' + str(sc)))
...
def read_code_file(file_path):
    code_lines = {}
    with open(file_path, encoding='gbk', errors='ignore') as fp:
...
# wv_model_li = Word2Vec.load('/opt/ReVeal-master/data/data/Word2Vec/li_et_al_wv')
wv_model_li = Word2Vec.load(args.model)
...
def extract_line_graph_data(
    ...
    for sc in range(1, shard_count + 1):
        # shard_file = open(os.path.join(base_dir, project, project + '.json.shard' + str(sc)))
        # shard_file = open(os.path.join(base_dir, '.json.shard' + str(sc)))
        shard_file = open(os.path.join(base_dir, '.shard' + str(sc)))
        shard_data = json.load(shard_file)
        try:
            for data in tqdm(shard_data):
