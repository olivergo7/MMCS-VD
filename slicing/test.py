...
def create_graph_from_nodes_and_edges(nodes_ids_to_nodes, edges, allowed_edge_types):
    ...
    graph.render('Code Dependency File', view=False)
    print(graph)
    pass
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', help='Output where slice results will be stored.', default='flow-output')
    parser.add_argument('--verbose', help='Show the slice results and the graph.', action='store_true')
    args = parser.parse_args()
    directory = './slice-output'
    for file_name in os.listdir(directory):
        try:
         slice_ln = int(args.line)
         code_file_path = os.path.join(directory, file_name)
         nodes_path = os.path.join('Devign_csv', file_name, 'nodes.csv')
         edges_path = os.path.join('Devign_csv', file_name, 'edges.csv')
    ...
    #
    # fo rward_sliced_lines = create_forward_slice(combined_graph, slice_ln)
    # fo rward_output_path = os.path.join(args.output, file_name + '.forward')
    # fp  = open(forward_output_path, 'w')
    # fo r ln in forward_sliced_lines:
    #      fp.write(code[ln] + '\n')
    # fp .close()
    #
    # ba ckward_sliced_lines = create_backward_slice(combined_graph, slice_ln)
    # ba ckward_output_path = os.path.join(args.output, file_name + '.backward')
    # fp  = open(backward_output_path, 'w')
    # fo r ln in backward_sliced_lines:
    #      fp.write(code[ln] + '\n')
    # fp .close()
    #
    # if  args.verbose:
